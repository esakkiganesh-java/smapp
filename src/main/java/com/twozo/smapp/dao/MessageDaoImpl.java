...
class MessageDaoImpl implements MessageDao {
		...
		final String query = "SELECT " +
		    ...
		    try (final Connection connection = dataSource.getConnection();
		        ...
		        while (resultSet.next()) {
                    ...
                    final MessageStatus status = MessageStatus.valueOf(resultSet.getString("status"));
	...
	public void markMessagesAsSeen(final int userId) {
		final String query = "UPDATE \"smapp_message_history\" SET status = ? WHERE receiver_id = ? AND status = 'delivered' ";

		try (final Connection connection = dataSource.getConnection();
			 final PreparedStatement stmt = connection.prepareStatement(query)) {

			 stmt.setString(1, String.valueOf(MessageStatus.seen));
			 stmt.setInt(2,userId);
			 stmt.executeUpdate();
		} catch (SQLException exception) {
